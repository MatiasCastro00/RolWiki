...
const MAP_TIMELINE_EVENT_TYPES = {
  seen: { label: "Visto", icon: "◉", color: "#7ed6c4" },
  found: { label: "Encontrado", icon: "✦", color: "#f2cf74" },
  lost: { label: "Perdido", icon: "×", color: "#e88778" },
  entered: { label: "Entró", icon: "→", color: "#8fb7ef" },
  left: { label: "Salió", icon: "←", color: "#bc9aeb" },
  moved: { label: "Pasó por", icon: "◆", color: "#d5e4df" },
};
...
let mapTimelinePlaybackFrame = null;
let mapTimelinePlaybackLastTime = 0;
let mapTimelineEditingUnlocked = false;
let mapTimelineContextMenu = null;
let mapTimelineEdit = null;
let mapClickChoice = null;
let mapTimelineDraft = null;
let mapUndoDeletion = null;
  ...
  return `
    <div class="wiki-graph ${compact ? "compact" : ""} ${wikiGraphExpanded ? "is-expanded" : ""}" data-wiki-graph data-can-manage="${canManage}" aria-label="Mapa de relaciones de la wiki">
      ...
      <div class="wiki-graph-toolbar">
        <span class="wiki-graph-pan-hint">Arrastrá el fondo para mover la cámara · rueda para zoom</span>
      ...
      ${canManage ? renderWikiGraphContextMenu() : ""}
      <div class="wiki-graph-legend"><span>${sorted.length} nodos</span><span>${edges.length} conexiones</span></div>
    </div>
  `;
}

function renderWikiGraphContextMenu() {
  if (!wikiGraphContextMenu) return "";
  return `<div class="wiki-graph-context-menu" style="left:${wikiGraphContextMenu.x}px;top:${wikiGraphContextMenu.y}px" data-wiki-graph-context-menu>
    <header><span>NUEVA FICHA</span><small>Elegí un tipo para crearla aquí</small></header>
    <div>${Object.entries(WIKI_CARD_TYPES).map(([key, type]) => `<button data-action="new-wiki-from-graph-type" data-wiki-type="${escapeAttr(key)}"><span>${type.icon}</span><strong>${escapeHtml(type.label)}</strong></button>`).join("")}</div>
  </div>`;
}
...
function teardownMapRuntime() {
  if (mapRuntime?.mapId && mapRuntime.snapshot) mapViewMemory.set(mapRuntime.mapId, mapRuntime.snapshot());
  if (mapTimelinePlaybackFrame !== null) cancelAnimationFrame(mapTimelinePlaybackFrame);
  mapTimelinePlaybackFrame = null;
  mapTimelinePlaybackLastTime = 0;
  mapRuntime = null;
}
...
function initializeMapRuntime() {
  ...
  const remembered = mapViewMemory.get(viewport.dataset.mapId);
  const state = { scale: remembered?.scale || 1, x: remembered?.x || 0, y: remembered?.y || 0, gesture: null };
  ...
  mapRuntime = {
    ...
    zoomBy,
    snapshot: () => ({ scale: state.scale, x: state.x, y: state.y }),
    reset: () => { state.scale = 1; state.x = 0; state.y = 0; apply(); },
  };
  ...
  viewport.addEventListener("pointerdown", (event) => {
    ...
    if (pointTrigger) {
      ...
      state.gesture = {
        kind: "move-point", pointerId: event.pointerId, pointId: pointTrigger.dataset.pointId,
        startX: event.clientX, startY: event.clientY, moved: false,
      };
      viewport.setPointerCapture(event.pointerId);
      return;
  ...
  viewport.addEventListener("pointermove", (event) => {
    ...
    if (gesture.kind === "move-point") {
      ...
      const point = canvas.querySelector(`.map-point[data-point-id="${gesture.pointId}"]`);
      if (point) { point.style.left = `${x}%`; point.style.top = `${y}%`; }
      gesture.x = x;
      gesture.y = y;
      event.preventDefault();
      return;
  ...
  initializeMapTimelinePlaybackRuntime();
}
...
function initializeMapTimelineHandleRuntime() {
  ...
  const finish = (event) => {
    ...
    drag = null;
    if (activeDrag.handle?.hasPointerCapture(event.pointerId)) activeDrag.handle.releasePointerCapture(event.pointerId);
    if (!activeDrag.moved) {
      ...
      if (index >= 0) {
        ...
        mapTimelineCursor = mapTimelinePosition(events[index], index);
        render();
      }
      return;
    }
    ...
    const events = mapTimelineEvents(mapsFor(campaignById(activeCampaignId)).find((item) => item.id === track.dataset.mapId), track.dataset.cardId);
    const position = mapTimelineLayout(events).positionAt(ratio);
    moveMapTimelineEventPosition(track.dataset.mapId, activeDrag.eventId, position, track.dataset.cardId);
  };

  track.addEventListener("pointerdown", (event) => {
    const handle = event.target.closest("[data-map-timeline-handle]");
    if (event.button !== 0 || !handle || track.dataset.editingUnlocked !== "true") return;
    drag = { pointerId: event.pointerId, eventId: handle.dataset.eventId, handle, startX: event.clientX, moved: false };
  ...
  track.addEventListener("contextmenu", (event) => {
    const handle = event.target.closest("[data-map-timeline-handle]");
    const campaign = campaignById(activeCampaignId);
    if (!handle || !campaign || !canManageCampaign(campaign, currentUser().id)) return;
    event.preventDefault();
    event.stopPropagation();
    openMapTimelineContextMenu(track.dataset.mapId, handle.dataset.eventId, event.clientX, event.clientY);
  });
}

function initializeMapTimelinePlaybackRuntime() {
  if (!mapTimelinePlaying || mapTimelinePlaybackFrame !== null) return;
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
  const events = mapTimelineEvents(map, selectedMapTrackCardId);
  if (!events.length) {
    mapTimelinePlaying = false;
    return;
  }
  const layout = mapTimelineLayout(events);
  mapTimelinePlaybackLastTime = performance.now();
  const duration = Math.max(4500, Math.min(30000, layout.span * 900));
  const tick = (time) => {
    if (!mapTimelinePlaying) {
      mapTimelinePlaybackFrame = null;
      return;
    }
    const elapsed = Math.max(0, time - mapTimelinePlaybackLastTime);
    mapTimelinePlaybackLastTime = time;
    const current = mapTimelineCursorFor(events);
    const next = Math.min(layout.maximum, current + layout.span * elapsed / duration);
    const range = document.querySelector("[data-map-timeline-range]");
    if (range) range.value = String(next);
    updateMapTimelineView(next);
    if (next >= layout.maximum - 0.001) {
      mapTimelineCursor = layout.maximum;
      mapTimelinePlaying = false;
      mapTimelinePlaybackFrame = null;
      render();
      return;
    }
    mapTimelinePlaybackFrame = requestAnimationFrame(tick);
  };
  mapTimelinePlaybackFrame = requestAnimationFrame(tick);
}
...
function setupWikiGraph(graph) {
  ...
  for (const edge of edges.filter((item) => item.isCharacterConnection)) {
    ...
    // La conexión dorada destaca al personaje jugador: el brillo pertenece a
    // su tarjeta, no al NPC que está del otro lado de la relación.
    if (sourceNode.element.classList.contains("player-node")) {
      edge.sourceGlow = document.createElement("span");
      edge.sourceGlow.className = "wiki-node-link-glow";
      sourceNode.element.append(edge.sourceGlow);
    }
    if (targetNode.element.classList.contains("player-node")) {
      edge.targetGlow = document.createElement("span");
      edge.targetGlow.className = "wiki-node-link-glow";
      targetNode.element.append(edge.targetGlow);
    }
  }
  ...
  graph.addEventListener("contextmenu", (event) => openWikiGraphContextMenu(event, graph));
  graph.addEventListener("wheel", (event) => zoomWikiGraph(event, graphState), { passive: false });
  renderWikiGraphFrame(graphState);
  return graphState;
}

function openWikiGraphContextMenu(event, graph) {
  if (graph.dataset.canManage !== "true" || event.target.closest(".wiki-node, .wiki-graph-toolbar, .wiki-graph-focus-actions, .wiki-graph-context-menu, .wiki-graph-legend")) return;
  event.preventDefault();
  event.stopPropagation();
  const rect = graph.getBoundingClientRect();
  wikiGraphContextMenu = {
    x: Math.max(8, Math.min(rect.width - 264, event.clientX - rect.left)),
    y: Math.max(8, Math.min(rect.height - 340, event.clientY - rect.top)),
  };
  render();
}
...
function zoomWikiGraph(event, graphState) {
  if (event.target.closest("button, input, select, textarea, a, .wiki-graph-context-menu")) return;
  event.preventDefault();
...
function startWikiNodeDrag(event, graphState) {
  ...
  const rect = graphState.graph.getBoundingClientRect();
  const pointerX = ((event.clientX - rect.left - graphState.camera.x) / graphState.camera.scale / Math.max(rect.width, 1)) * 100;
  const pointerY = ((event.clientY - rect.top - graphState.camera.y) / graphState.camera.scale / Math.max(rect.height, 1)) * 100;
  graphState.dragging = {
    node,
    pointerId: event.pointerId,
  ...
  function handleWikiNodeDragMove(moveEvent) {
    ...
    const nextX = ((moveEvent.clientX - moveRect.left - graphState.camera.x) / graphState.camera.scale / Math.max(moveRect.width, 1)) * 100 + graphState.dragging.offsetX;
    const nextY = ((moveEvent.clientY - moveRect.top - graphState.camera.y) / graphState.camera.scale / Math.max(moveRect.height, 1)) * 100 + graphState.dragging.offsetY;
...
function mapDisplayFor(card) {
  const display = card?.mapDisplay && typeof card.mapDisplay === "object" ? card.mapDisplay : {};
  return {
    aliases: display.aliases && typeof display.aliases === "object" ? display.aliases : {},
    properties: display.properties && typeof display.properties === "object" ? display.properties : {},
    content: display.content && typeof display.content === "object" ? display.content : {},
  };
}

function mapDisplayEnabled(display, group, id) {
...
function mapTimelineLayout(events) {
  ...
  const currentMonth = mapTimelineMonthSerial(currentMapTimelineMonth());
  const maximum = Math.max(highest, currentMonth) > minimum ? Math.max(highest, currentMonth) : minimum + 1;
  return {
    minimum,
    maximum,
    span: Math.max(1, maximum - minimum),
    percent(event, index) { return ((mapTimelinePosition(event, index) - minimum) / Math.max(1, maximum - minimum)) * 100; },
    positionAt(ratio) { return minimum + Math.max(0, Math.min(1, ratio)) * Math.max(1, maximum - minimum); },
  };
}
...
function openMapTimelineContextMenu(mapId, eventId, clientX, clientY) {
  ...
  mapTimelineContextMenu = {
    mapId,
    eventId,
    left: Math.max(8, Math.min(window.innerWidth - 235, clientX + 8)),
    top: Math.max(8, Math.min(window.innerHeight - 160, clientY + 8)),
  };
...
function mapTimelineType(type) {
  return MAP_TIMELINE_EVENT_TYPES[type] || MAP_TIMELINE_EVENT_TYPES.seen;
}
...
function mapTimelineCursorFor(events) {
  const layout = mapTimelineLayout(events);
  const requested = mapTimelineCursor === null ? layout.maximum : Number(mapTimelineCursor);
  return Math.max(layout.minimum, Math.min(layout.maximum, Number.isFinite(requested) ? requested : layout.maximum));
}

function mapTimelineReadout(events, cursor) {
  if (!events.length) return "Sin hitos";
  return mapTimelineFormattedMonth(mapTimelineMonthValue(cursor));
}

function mapTimelineStepForMonth(events, cursor) {
  if (!events.length) return 0;
  const value = Number(cursor);
  const first = mapTimelinePosition(events[0], 0);
  if (value <= first) return 0;
  for (let index = 1; index < events.length; index += 1) {
    const previous = mapTimelinePosition(events[index - 1], index - 1);
    const next = mapTimelinePosition(events[index], index);
    if (value > next) continue;
    const ratio = next === previous ? 1 : Math.max(0, Math.min(1, (value - previous) / (next - previous)));
    return index - 1 + ratio;
  }
  return events.length;
}

function mapTimelinePositionAt(events, cursor) {
  if (!events.length || cursor < 0) return null;
  const last = events[events.length - 1];
  if (cursor >= events.length - 1 || events.length === 1) {
    return { x: Number(last.x), y: Number(last.y), event: last };
  }
  const startIndex = Math.max(0, Math.floor(cursor));
  const endIndex = Math.min(events.length - 1, startIndex + 1);
  const previous = events[startIndex];
  const next = events[endIndex];
  const ratio = Math.max(0, Math.min(1, cursor - startIndex));
  return {
    x: Number(previous.x) + (Number(next.x) - Number(previous.x)) * ratio,
    y: Number(previous.y) + (Number(next.y) - Number(previous.y)) * ratio,
    event: ratio >= 1 ? next : previous,
  };
}
...
function renderMapsPage(campaign, role, canManage) {
  ...
  if (selected && !(selected.points || []).some((point) => point.id === selectedMapPointId)) selectedMapPointId = null;
  if (selected && selectedMapTrackCardId && !wikiCardsFor(campaign).some((card) => card.id === selectedMapTrackCardId)) selectedMapTrackCardId = null;
  ...
  const sidebar = `
    ...
    ${selected ? renderMapTrackCards(campaign, selected) : ""}
  `;

  const main = selected
    ? `<div class="map-experience">${renderMapStage(campaign, selected, canManage, isPlayer, settings)}${renderMapTimelinePanel(campaign, selected, canManage)}</div>`
...
function renderMapTrackCards(campaign, map) {
  const cards = mapTrackCards(campaign, map);
  return `<section class="map-track-browser">
    <div class="map-browser-head"><span>FICHAS EN EL MAPA</span><small>${cards.length}</small></div>
    <div class="map-track-list">
      ${cards.length ? cards.map((card) => {
        const events = mapTimelineEvents(map, card.id);
        const latest = events[events.length - 1];
        const latestType = latest ? mapTimelineType(latest.type) : null;
        return `<button class="map-track-card ${card.id === selectedMapTrackCardId ? "active" : ""}" data-action="select-map-track" data-id="${card.id}">
          ${renderWikiThumb(card)}
          <span><strong>${escapeHtml(card.title)}</strong><small>${latest ? `${escapeHtml(latestType.label)} · ${escapeHtml(latest.placeName || "En el mapa")}` : "Sin hitos todavía"}</small></span>
          <em>${events.length || "+"}</em>
        </button>`;
      }).join("") : `<div class="map-browser-empty">Creá fichas para seguir personajes y objetos.</div>`}
    </div>
  </section>`;
}
...
function renderMapStage(campaign, map, canManage, isPlayer, settings) {
  ...
  const timelineEvents = selectedMapTrackCardId ? mapTimelineEvents(map) : [];
  const timelineMonth = mapTimelineCursorFor(timelineEvents);
  const timelineCursor = mapTimelineStepForMonth(timelineEvents, timelineMonth);
  const mapImage = mapImageFor(campaign, map);
  const canDraw = canManage || isPlayer && settings.playersCanDraw;
  const pointPicker = mapPointPicker?.mapId === map.id ? renderMapPointPicker(campaign, mapPointPicker) : "";
...
function renderMapClickChoice(campaign, choice) {
  const card = wikiCardsFor(campaign).find((item) => item.id === choice.cardId);
  if (!card) return "";
  return `<aside class="map-click-choice" style="left:${Number(choice.left)}px;top:${Number(choice.top)}px">
    <header><div><small>AGREGAR AL MAPA</small><strong>${escapeHtml(card.title)}</strong></div><button data-action="cancel-map-click-choice" aria-label="Cerrar">×</button></header>
    <p>¿Qué querés crear en este punto?</p>
    <div><button data-action="create-selected-map-card"><span>▣</span><strong>Tarjeta</strong><small>Punto interactivo con la ficha</small></button><button data-action="start-map-timeline-draft"><span>◆</span><strong>Hito</strong><small>Nuevo paso de su recorrido</small></button></div>
  </aside>`;
}

function renderMapTimelineRoute(events, cursor) {
  if (!events.length) return "";
  const segments = events.slice(1).map((event, index) => {
    const previous = events[index];
    const start = index;
    const end = index + 1;
    const ratio = cursor <= start ? 0 : cursor >= end ? 1 : cursor - start;
    const x2 = Number(previous.x) + (Number(event.x) - Number(previous.x)) * ratio;
    const y2 = Number(previous.y) + (Number(event.y) - Number(previous.y)) * ratio;
    return `<line class="map-timeline-segment" data-timeline-segment data-start-step="${start}" data-end-step="${end}" data-x1="${Number(previous.x)}" data-y1="${Number(previous.y)}" data-x2="${Number(event.x)}" data-y2="${Number(event.y)}" x1="${Number(previous.x) * 10}" y1="${Number(previous.y) * 10}" x2="${x2 * 10}" y2="${y2 * 10}" ${ratio <= 0 ? "hidden" : ""} />`;
  }).join("");
  return `<g class="map-timeline-route" data-map-timeline-route>${segments}</g>`;
}

function renderMapTimelineMarkers(campaign, map, events, cursor, canManage) {
  if (!events.length) return "";
  const card = wikiCardsFor(campaign).find((item) => item.id === selectedMapTrackCardId);
  const position = mapTimelinePositionAt(events, cursor);
  ...
  const current = position ? `<div class="map-timeline-current" data-map-timeline-current style="left:${position.x}%;top:${position.y}%">${card?.imageUrl ? `<img src="${escapeAttr(card.imageUrl)}" alt="" />` : `<span>${escapeHtml(card ? wikiType(card).icon : "◆")}</span>`}</div>` : `<div class="map-timeline-current" data-map-timeline-current hidden></div>`;
  return `${markers}${current}`;
}

function renderMapTimelineEventInfo(event, type, canManage, index) {
  return `<aside class="map-timeline-event-info">
    <button class="map-point-info-close" data-action="select-map-timeline-event" data-id="${event.id}" aria-label="Cerrar">×</button>
    <small>${escapeHtml(type.label)} · ${escapeHtml(mapTimelineMomentLabel(event, index))}</small>
    <strong>${escapeHtml(event.placeName || "Lugar sin nombre")}</strong>
    ${event.note ? `<p>${escapeHtml(event.note)}</p>` : ""}
    ${canManage ? `<button class="map-timeline-delete" data-action="delete-map-timeline-event" data-id="${event.id}">Eliminar hito</button>` : ""}
  </aside>`;
}

function renderMapTimelineDraft(campaign, draft) {
  const card = wikiCardsFor(campaign).find((item) => item.id === draft.cardId);
  if (!card) return "";
  return `<form class="map-timeline-editor" data-form="map-timeline-event" style="left:${Number(draft.left)}px;top:${Number(draft.top)}px">
...
function renderMapTimelineContextMenu(campaign, map, canManage) {
  const menu = mapTimelineContextMenu;
  if (!canManage || !menu || menu.mapId !== map.id) return "";
  const event = (map.timelineEvents || []).find((item) => item.id === menu.eventId);
  if (!event) return "";
  return `<aside class="map-timeline-context-menu" data-map-timeline-context-menu style="left:${Number(menu.left)}px;top:${Number(menu.top)}px">
    <small>HITO</small><strong>${escapeHtml(event.placeName || mapTimelineMomentLabel(event))}</strong>
    <button data-action="change-map-timeline-date" data-id="${event.id}">Cambiar fecha</button>
    <button data-action="edit-map-timeline-event" data-id="${event.id}">Editar</button>
    <button data-action="set-map-timeline-first" data-id="${event.id}">Setear como primer hito</button>
  </aside>`;
}

function renderMapTimelineEdit(campaign, map, canManage) {
  const draft = mapTimelineEdit;
  if (!canManage || !draft || draft.mapId !== map.id) return "";
  const event = (map.timelineEvents || []).find((item) => item.id === draft.eventId);
  if (!event) return "";
  const isDateOnly = draft.mode === "date";
  return `<div class="map-timeline-modal-backdrop"><form class="map-timeline-modal" data-form="map-timeline-event-edit">
...
function renderMapTimelinePanel(campaign, map, canManage) {
  ...
  const layout = mapTimelineLayout(events);
  const cursorStep = mapTimelineStepForMonth(events, cursor);
  const focusedIndex = Math.min(events.length - 1, Math.max(0, Math.round(cursorStep)));
  const cursorPercent = ((cursor - layout.minimum) / layout.span) * 100;
  const selectedIndex = events.findIndex((event) => event.id === selectedMapTimelineEventId);
  ...
  return `<section class="map-timeline-panel" data-map-timeline-panel>
    <header><div>${renderWikiThumb(card)}<span><small>LÍNEA DE TIEMPO</small><strong>${escapeHtml(card.title)}</strong></span></div>${selectedEvent && canManage ? `<button class="map-timeline-selected-edit" data-action="edit-map-timeline-direct" data-map-id="${map.id}" data-id="${selectedEvent.id}">✎ Editar</button>` : ""}</header>
    <div class="map-timeline-scrubber">
      ...
      <div class="map-timeline-track" data-map-timeline-track data-map-id="${map.id}" data-card-id="${card.id}" data-editing-unlocked="${mapTimelineEditingUnlocked}">
        <input data-map-timeline-range type="range" min="${layout.minimum}" max="${layout.maximum}" step="0.01" value="${cursor}" aria-label="Momento del recorrido" />
        <output class="map-timeline-date ${cursorPercent < 15 ? "align-start" : cursorPercent > 85 ? "align-end" : ""}" data-map-timeline-date style="left:${cursorPercent}%">${escapeHtml(mapTimelineReadout(events, cursor))}</output>
        <button class="map-timeline-play ${mapTimelinePlaying ? "playing" : ""}" data-action="toggle-map-timeline-play" aria-pressed="${mapTimelinePlaying}" title="${mapTimelinePlaying ? "Pausar recorrido" : "Reproducir recorrido"}">${mapTimelinePlaying ? "⏸" : "▶"}</button>
        ${canManage ? `<button class="map-timeline-lock ${mapTimelineEditingUnlocked ? "unlocked" : ""}" data-action="toggle-map-timeline-lock" aria-pressed="${mapTimelineEditingUnlocked}" title="${mapTimelineEditingUnlocked ? "Bloquear posición de hitos" : "Desbloquear para mover hitos"}">${mapTimelineEditingUnlocked ? "🔓" : "🔒"}</button>` : ""}
        <div class="map-timeline-handles">${events.map((event, index) => { const type = mapTimelineType(event.type); const position = layout.percent(event, index); const milestoneName = event.placeName || mapTimelineMomentLabel(event, index); const selected = selectedMapTimelineEventId === event.id; return `<button class="map-timeline-handle ${selected ? "selected" : ""} ${mapTimelineEditingUnlocked ? "unlocked" : ""}" data-map-timeline-handle data-action="jump-map-timeline-event" data-id="${event.id}" data-event-id="${event.id}" style="left:${position}%;--event-color:${type.color}" aria-label="${escapeAttr(`Hito: ${milestoneName}`)}"><span>${type.icon}</span><span class="map-timeline-handle-tooltip"><strong>${escapeHtml(milestoneName)}</strong></span></button>`; }).join("")}</div>
        <div class="map-timeline-point-labels">${events.map((event, index) => { const type = mapTimelineType(event.type); const position = layout.percent(event, index); return `<span data-map-timeline-label data-event-step="${index}" style="left:${position}%;--label-lane:${index % 2};--event-color:${type.color}" ${Math.abs(index - focusedIndex) > 2 ? "hidden" : ""}>${escapeHtml(event.placeName || type.label)}</span>`; }).join("")}</div>
        <output class="map-timeline-drag-readout" data-map-timeline-drag-readout hidden></output>
      </div>
...
function updateMapTimelineView(value) {
  const cursorMonth = Number(value);
  if (!Number.isFinite(cursorMonth)) return;
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
  const events = mapTimelineEvents(map, selectedMapTrackCardId);
  const cursor = mapTimelineStepForMonth(events, cursorMonth);
  mapTimelineCursor = cursorMonth;
  const layout = mapTimelineLayout(events);
  const dateOutput = document.querySelector("[data-map-timeline-date]");
  if (dateOutput) {
    dateOutput.textContent = mapTimelineReadout(events, cursorMonth);
    const datePercent = ((cursorMonth - layout.minimum) / layout.span) * 100;
    dateOutput.style.left = `${datePercent}%`;
    dateOutput.classList.toggle("align-start", datePercent < 15);
    dateOutput.classList.toggle("align-end", datePercent > 85);
  }
  document.querySelectorAll("[data-timeline-segment]").forEach((segment) => {
    const start = Number(segment.dataset.startStep);
    const end = Number(segment.dataset.endStep);
    const ratio = cursor <= start ? 0 : cursor >= end || end === start ? 1 : (cursor - start) / (end - start);
    const x1 = Number(segment.dataset.x1);
    const y1 = Number(segment.dataset.y1);
    const x2 = x1 + (Number(segment.dataset.x2) - x1) * ratio;
    const y2 = y1 + (Number(segment.dataset.y2) - y1) * ratio;
    segment.toggleAttribute("hidden", ratio <= 0);
    segment.setAttribute("x2", String(x2 * 10));
    segment.setAttribute("y2", String(y2 * 10));
  });
  const markers = [...document.querySelectorAll("[data-map-timeline-event]")]
    .sort((a, b) => Number(a.dataset.eventStep) - Number(b.dataset.eventStep));
  markers.forEach((marker) => marker.toggleAttribute("hidden", Number(marker.dataset.eventStep) > cursor));
  const current = document.querySelector("[data-map-timeline-current]");
  if (current && markers.length) {
    const firstStep = Number(markers[0].dataset.eventStep);
    if (cursor < firstStep) {
      current.hidden = true;
    } else {
      let x = Number(markers[markers.length - 1].dataset.eventX);
      let y = Number(markers[markers.length - 1].dataset.eventY);
      for (let index = 1; index < markers.length; index += 1) {
        const previous = markers[index - 1];
        const next = markers[index];
        const start = Number(previous.dataset.eventStep);
        const end = Number(next.dataset.eventStep);
        if (cursor > end) continue;
        const ratio = end === start ? 1 : Math.max(0, Math.min(1, (cursor - start) / (end - start)));
        x = Number(previous.dataset.eventX) + (Number(next.dataset.eventX) - Number(previous.dataset.eventX)) * ratio;
        y = Number(previous.dataset.eventY) + (Number(next.dataset.eventY) - Number(previous.dataset.eventY)) * ratio;
        break;
      }
  ...
  const labels = [...document.querySelectorAll("[data-map-timeline-label]")];
  const focusedIndex = Math.min(labels.length - 1, Math.max(0, Math.round(cursor)));
  labels.forEach((label) => label.toggleAttribute("hidden", Math.abs(Number(label.dataset.eventStep) - focusedIndex) > 2));
}
...
function renderMapPointInfo(campaign, point) {
  const card = point.card;
  const display = mapDisplayFor(card);
  const aliases = (card.aliases || []).filter((alias) => normalizeSearchText(alias) !== normalizeSearchText(card.title) && mapDisplayEnabled(display, "aliases", alias));
  const properties = normalizedWikiPropertyItems(card).filter((item) => item.value.trim() && mapDisplayEnabled(display, "properties", item.id));
  const content = normalizedWikiContentBlocks(card).filter((block) => block.text.trim() && mapDisplayEnabled(display, "content", block.id));
  const linkedMap = mapsFor(campaign).find((item) => item.cardId === card.id);
  const linkedMapImage = mapImageFor(campaign, linkedMap);
  return `<aside class="map-point-info" data-map-point-info>
    <button class="map-point-info-close" data-action="toggle-map-point" data-id="${point.id}" aria-label="Cerrar">x</button>
    <header>${card.imageUrl ? `<img src="${escapeAttr(card.imageUrl)}" alt="" />` : `<span>${escapeHtml(wikiType(card).icon)}</span>`}<div><small>${escapeHtml(wikiType(card).label)}</small><strong>${escapeHtml(card.title)}</strong></div></header>
    ${aliases.length ? `<p class="map-point-aliases">${aliases.map(escapeHtml).join(" · ")}</p>` : ""}
    ${properties.map((item) => `<p class="map-point-property"><small>${escapeHtml(item.label)}</small><span>${escapeHtml(item.value)}</span></p>`).join("")}
    ${content.map((block) => `<p class="map-point-content"><small>${escapeHtml(block.title)}</small><span>${escapeHtml(block.text).slice(0, 220)}${block.text.length > 220 ? "..." : ""}</span></p>`).join("")}
    ${linkedMapImage ? `<button class="map-linked-preview" data-action="go-to-map" data-id="${linkedMap.id}"><img src="${escapeAttr(linkedMapImage)}" alt="Vista previa de ${escapeAttr(card.title)}" /><span>Ir a este mapa</span></button>` : ""}
  </aside>`;
}

function renderMapPointPicker(campaign, picker) {
  const cards = wikiCardsFor(campaign);
  return `<aside class="map-context-picker" style="left:${picker.left}px;top:${picker.top}px" data-map-context-picker>
    <header><strong>Vincular tarjeta</strong><button data-action="close-map-picker" aria-label="Cerrar">x</button></header>
    <label class="map-search"><span>Buscar</span><input data-map-link-search type="search" placeholder="Buscar tarjetas" autofocus /></label>
...
function wikiAliasChip(alias, index, display = {}) {
  const visible = mapDisplayEnabled(display, "aliases", alias);
  return `<span class="wiki-alias-chip" data-alias-chip><input type="hidden" name="alias_${index}" value="${escapeAttr(alias)}" /><span>${escapeHtml(alias)}</span><label class="map-display-toggle" title="Mostrar alias en el mapa"><input name="map_show_alias_${index}" type="checkbox" ${visible ? "checked" : ""} />⌖</label><button type="button" data-action="remove-wiki-alias" aria-label="Quitar alias ${escapeAttr(alias)}">×</button></span>`;
}
...
function wikiPropertyEditorRow(item, index, display = {}) {
  const icon = item.icon || "◆";
  const visible = mapDisplayEnabled(display, "properties", item.id);
  return `<div class="wiki-property-editor" data-property-row>
    ...
    <label class="map-display-toggle" title="Mostrar esta propiedad en el mapa"><input name="map_show_property_${escapeAttr(item.id)}" type="checkbox" ${visible ? "checked" : ""} />⌖</label>
...
function wikiContentEditorBlock(block, index, display = {}) {
  const type = WIKI_CONTENT_TYPES[block.type] || WIKI_CONTENT_TYPES.text;
  ...
  const linkedMapImage = mapImageFor(campaign, linkedMap);
  const visible = mapDisplayEnabled(display, "content", block.id);
  return `<article class="wiki-content-editor" data-content-block>
    ...
    <header><span>${type.icon} ${escapeHtml(type.label)}</span><label class="map-display-toggle" title="Mostrar este contenido en el mapa"><input name="map_show_content_${escapeAttr(block.id)}" type="checkbox" ${visible ? "checked" : ""} />⌖</label><button type="button" data-action="remove-content-block" aria-label="Quitar bloque">×</button></header>
...
function renderWikiModal(pageId) {
  ...
  const contentBlocks = normalizedWikiContentBlocks(page);
  const mapDisplay = mapDisplayFor(page);
  const assignedPlayer = state.users.find((user) => user.id === page.playerId);
          ...
          <div class="wiki-form-columns">
            <div class="wiki-form-main">
              ...
              <section class="wiki-editor-section">
                ...
                <div class="wiki-property-editors" data-property-list>${properties.map((item, index) => wikiPropertyEditorRow(item, index, mapDisplay)).join("")}</div>
              ...
              <section class="wiki-editor-section wiki-content-section">
                ...
                <div class="wiki-content-editors" data-content-list>${contentBlocks.map((block, index) => wikiContentEditorBlock(block, index, mapDisplay)).join("")}</div>
...
document.addEventListener("submit", async (event) => {
  ...
  if (formType === "map-timeline-event") {
    saveMapTimelineEvent(data);
    return;
  }

  if (formType === "map-timeline-event-edit") {
    saveMapTimelineEventEdit(data);
    return;
  }
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "cancel-map-click-choice") {
    mapClickChoice = null;
    render();
  }

  if (action === "create-selected-map-card") {
    const choice = mapClickChoice;
    if (!choice) return;
    mapClickChoice = null;
    addMapPoint(choice.mapId, choice.cardId, choice.x, choice.y);
  }

  if (action === "start-map-timeline-draft") {
    if (!mapClickChoice) return;
    mapTimelineDraft = { ...mapClickChoice, top: mapClickChoice.editorTop ?? mapClickChoice.top };
    mapClickChoice = null;
    selectedMapTimelineEventId = null;
    render();
  }

  if (action === "cancel-map-timeline-draft") {
    mapTimelineDraft = null;
    render();
  }

  if (action === "change-map-timeline-date" || action === "edit-map-timeline-event") {
    const menu = mapTimelineContextMenu;
    if (!menu) return;
    mapTimelinePlaying = false;
    mapTimelineEdit = { mapId: menu.mapId, eventId: id, mode: action === "change-map-timeline-date" ? "date" : "edit" };
    mapTimelineContextMenu = null;
    render();
  }

  if (action === "edit-map-timeline-direct") {
    const mapId = target.dataset.mapId || target.closest("[data-map-timeline-track]")?.dataset.mapId || selectedMapId;
    if (!mapId || !id) return;
    mapTimelinePlaying = false;
    mapTimelineEdit = { mapId, eventId: id, mode: "edit" };
    mapTimelineContextMenu = null;
    render();
    return;
  }

  if (action === "cancel-map-timeline-edit") {
    mapTimelineEdit = null;
    render();
  }

  if (action === "set-map-timeline-first") {
    setMapTimelineEventFirst(id);
  }
  ...
  if (action === "jump-map-timeline-event") {
    const campaign = campaignById(activeCampaignId);
    const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
    const eventIndex = mapTimelineEvents(map).findIndex((item) => item.id === id);
    if (eventIndex >= 0) {
      const events = mapTimelineEvents(map);
      mapTimelinePlaying = false;
      selectedMapTimelineEventId = id;
      mapTimelineCursor = mapTimelinePosition(events[eventIndex], eventIndex);
      render();
    }
  }

  if (action === "toggle-map-timeline-play") {
    const campaign = campaignById(activeCampaignId);
    const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
    const events = mapTimelineEvents(map, selectedMapTrackCardId);
    if (!events.length) return;
    ...
    if (mapTimelinePlaying) {
      ...
      mapTimelineCursor = layout.minimum;
      selectedMapTimelineEventId = null;
  ...
  if (action === "delete-map-timeline-event") deleteMapTimelineEvent(id);
  ...
  if (action === "toggle-map-point") {
    if (Date.now() < mapIgnorePointClickUntil) return;
    selectedMapPointId = selectedMapPointId === id ? null : id;
    mapPointPicker = null;
    mapClickChoice = null;
    render();
  }

  if (action === "add-map-point") addMapPoint(mapPointPicker?.mapId || editing?.mapId, id, mapPointPicker?.x ?? editing?.x, mapPointPicker?.y ?? editing?.y);

  if (action === "close-map-picker") {
    mapPointPicker = null;
    render();
  }

  if (action === "toggle-map-elements") {
    mapElementsHidden = !mapElementsHidden;
    selectedMapPointId = null;
    render();
  }
  ...
  if (action === "new-wiki-from-graph") {
    wikiGraphContextMenu = null;
    editing = { type: "wiki-type", folder: "Sin carpeta", fromGraph: true };
    render();
  }

  if (action === "new-wiki-from-graph-type") {
    wikiGraphContextMenu = null;
    editing = { type: "wiki", id: null, wikiType: target.dataset.wikiType, folder: "Sin carpeta", fromGraph: true };
    render();
  }
...
document.addEventListener("contextmenu", (event) => {
  ...
  const timelineTarget = event.target.closest("[data-map-timeline-handle], .map-timeline-event");
  if (timelineTarget && campaign && canManageCampaign(campaign, currentUser().id)) {
    const mapId = timelineTarget.closest("[data-map-viewport]")?.dataset.mapId || selectedMapId;
    const eventId = timelineTarget.dataset.eventId;
    if (!mapId || !eventId) return;
    event.preventDefault();
    openMapTimelineContextMenu(mapId, eventId, event.clientX, event.clientY);
    return;
  }
  const viewport = event.target.closest("[data-map-viewport]");
  if (!viewport || viewport.dataset.canManage !== "true" || event.target.closest(".map-point, .map-upload-button, .map-overlay-tools, .map-context-picker, .map-click-choice, .map-timeline-editor, .map-timeline-event")) return;
  const canvas = viewport.querySelector("[data-map-canvas]");
  if (!canvas) return;
  event.preventDefault();
  const rect = canvas.getBoundingClientRect();
...
document.addEventListener("click", (event) => {
  ...
  if (wikiGraphContextMenu && !event.target.closest("[data-wiki-graph-context-menu]")) {
    wikiGraphContextMenu = null;
    render();
  }
});
...
function deleteSelectedMapElement() {
  if (activeTab !== "maps") return false;
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
  if (!campaign || !map || !canManageCampaign(campaign, currentUser().id)) return false;

  if (selectedMapTimelineEventId) {
    const index = (map.timelineEvents || []).findIndex((item) => item.id === selectedMapTimelineEventId);
    if (index < 0) return false;
    const item = structuredClone(map.timelineEvents[index]);
    mapUndoDeletion = { campaignId: campaign.id, mapId: map.id, kind: "timeline", item, index };
    map.timelineEvents.splice(index, 1);
    selectedMapTimelineEventId = null;
    mapTimelineCursor = null;
    saveState();
    render();
    showToast("Hito eliminado. Ctrl+Z para restaurarlo.");
    return true;
  }

  if (selectedMapPointId) {
    const index = (map.points || []).findIndex((item) => item.id === selectedMapPointId);
    if (index < 0) return false;
    const item = structuredClone(map.points[index]);
    mapUndoDeletion = { campaignId: campaign.id, mapId: map.id, kind: "point", item, index };
    map.points.splice(index, 1);
    selectedMapPointId = null;
    saveState();
    render();
    showToast("Punto eliminado. Ctrl+Z para restaurarlo.");
    return true;
  }
  return false;
}

function undoMapDeletion() {
  const undo = mapUndoDeletion;
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === undo?.mapId);
  if (!undo || !campaign || campaign.id !== undo.campaignId || !map || !canManageCampaign(campaign, currentUser().id)) return false;
  const collection = undo.kind === "timeline" ? (map.timelineEvents ||= []) : (map.points ||= []);
  if (collection.some((item) => item.id === undo.item.id)) return false;
  collection.splice(Math.max(0, Math.min(undo.index, collection.length)), 0, structuredClone(undo.item));
  if (undo.kind === "timeline") {
    selectedMapTimelineEventId = undo.item.id;
    selectedMapTrackCardId = undo.item.cardId;
    const restoredEvents = mapTimelineEvents(map, undo.item.cardId);
    const restoredIndex = restoredEvents.findIndex((item) => item.id === undo.item.id);
    mapTimelineCursor = restoredIndex >= 0 ? mapTimelinePosition(restoredEvents[restoredIndex], restoredIndex) : null;
  } else selectedMapPointId = undo.item.id;
  mapUndoDeletion = null;
  saveState();
  render();
  showToast(undo.kind === "timeline" ? "Hito restaurado." : "Punto restaurado.");
  return true;
}

document.addEventListener("keydown", (event) => {
  const editableTarget = event.target.closest("input, textarea, select, [contenteditable='true']");
  if (!editableTarget && (event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "z" && undoMapDeletion) {
    if (undoMapDeletion()) event.preventDefault();
    return;
  }
  if (!editableTarget && event.key === "Delete") {
    if (deleteSelectedMapElement()) event.preventDefault();
    return;
  }
...
document.addEventListener("input", (event) => {
  const mapTimelineInput = event.target.closest("[data-map-timeline-range]");
  if (mapTimelineInput) {
    if (mapTimelinePlaying) {
      mapTimelinePlaying = false;
      if (mapTimelinePlaybackFrame !== null) cancelAnimationFrame(mapTimelinePlaybackFrame);
      mapTimelinePlaybackFrame = null;
      const playButton = document.querySelector("[data-action='toggle-map-timeline-play']");
      if (playButton) {
        playButton.textContent = "▶";
        playButton.classList.remove("playing");
        playButton.setAttribute("aria-pressed", "false");
        playButton.title = "Reproducir recorrido";
      }
    }
    updateMapTimelineView(mapTimelineInput.value);
    return;
  }
...
async function saveWikiPage(data) {
  ...
  const aliasDisplay = Object.entries(data)
    .filter(([key]) => /^alias_/.test(key))
    .reduce((result, [key, value]) => ({ ...result, [String(value || "").trim()]: Boolean(data[`map_show_alias_${key.slice(6)}`]) }), {});
  const mapDisplay = {
    aliases: aliasDisplay,
    properties: Object.fromEntries(propertyItems.map((item) => [item.id, Boolean(data[`map_show_property_${item.id}`])])),
    content: Object.fromEntries(contentBlocks.map((block) => [block.id, Boolean(data[`map_show_content_${block.id}`])])),
  };
...
function createMapFromCard(cardId) {
  ...
  const map = { id: uid("map"), cardId, points: [], playerStrokes: [], timelineEvents: [], createdAt: Date.now() };
...
function addMapPoint(mapId, cardId, x, y) {
  ...
  const point = { id: uid("point"), cardId, x: Math.max(0, Math.min(100, Number(x))), y: Math.max(0, Math.min(100, Number(y))) };
  map.points = [...(map.points || []), point];
  editing = null;
  mapPointPicker = null;
  mapClickChoice = null;
...
function moveMapPoint(mapId, pointId, x, y) {
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === mapId);
  const point = map?.points?.find((item) => item.id === pointId);
  if (!campaign || !map || !point || !canManageCampaign(campaign, currentUser().id)) return;
  point.x = Math.max(0, Math.min(100, Number(x)));
  point.y = Math.max(0, Math.min(100, Number(y)));
  saveState();
  render();
}

function moveMapTimelineEventPosition(mapId, eventId, position, cardId = selectedMapTrackCardId) {
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === mapId);
  ...
  const reorderedIndex = reordered.findIndex((event) => event.id === eventId);
  mapTimelineCursor = reorderedIndex >= 0 ? mapTimelinePosition(reordered[reorderedIndex], reorderedIndex) : null;
  saveState();
  render();
...
function saveMapTimelineEvent(data) {
  ...
  map.timelineEvents = [...(Array.isArray(map.timelineEvents) ? map.timelineEvents : []), event];
  selectedMapTrackCardId = card.id;
  ...
  saveState();
  render();
  showToast(`Hito agregado al recorrido de ${card.title}.`);
}

function saveMapTimelineEventEdit(data) {
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
  ...
  if (data.mode === "edit") {
    ...
    event.note = String(data.note || "").trim();
  }
  selectedMapTimelineEventId = event.id;
  mapTimelineEdit = null;
  saveState();
...
function setMapTimelineEventFirst(eventId) {
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
  const event = map?.timelineEvents?.find((item) => item.id === eventId);
  if (!campaign || !map || !event || !canManageCampaign(campaign, currentUser().id)) return;
  ...
  selectedMapTimelineEventId = event.id;
  mapTimelineCursor = event.timelinePosition;
  mapTimelineContextMenu = null;
  saveState();
  render();
  showToast("El hito ahora es el primero del recorrido.");
}

function deleteMapTimelineEvent(eventId) {
  const campaign = campaignById(activeCampaignId);
  const map = mapsFor(campaign).find((item) => item.id === selectedMapId);
  if (!campaign || !map || !canManageCampaign(campaign, currentUser().id)) return;
  const event = (map.timelineEvents || []).find((item) => item.id === eventId);
  if (!event || !window.confirm("¿Eliminar este hito del recorrido?")) return;
  map.timelineEvents = map.timelineEvents.filter((item) => item.id !== eventId);
  selectedMapTimelineEventId = null;
  mapTimelineCursor = null;
  saveState();
  render();
...
function deleteWikiPage(id) {
  ...
  campaign.maps = mapsFor(campaign)
    ...
    .map((map) => ({
      ...map,
      points: (map.points || []).filter((point) => point.cardId !== id),
      timelineEvents: (map.timelineEvents || []).filter((event) => event.cardId !== id),
    }));
  ...
  if (selectedMapTrackCardId === id) selectedMapTrackCardId = null;
  saveState();
  render();
